  ...
  return (
    <div className="bg-gray-900 w-full max-h-40 h-14 text-white py-8 top-0 flex items-center px-4 ">
      ...
      <div className="mr-6 py-1  text-base  w-[20%]  grid gap-3 grid-cols-3 ">
        <Link to={`${loginstatus ? '/register' :'/login'}`}>
        <button className="px-3  border rounded-md cursor-pointer bg-white text-gray-900 font-bold py-1 hover:bg-gray-300  text-center justify-self-end col-span-1 col-start-3">{loginstatus ? loginstatus :'Login' }</button>
        </Link>
      </div>
    </div>
  );
