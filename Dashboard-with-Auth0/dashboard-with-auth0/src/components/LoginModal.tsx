...
export default function LoginModal({ onClose }: { onClose: () => void }) {
  ...
  async function loginWithGoogle(e: React.MouseEvent) {
    ...
    try {
      ...
      const emailRef = doc(db, "userEmails", user.email.toLowerCase());
      const emailSnap = await getDoc(emailRef);

      if (emailSnap.exists()) {
        throw new Error("This email is already registered. Please login with email/password.");
      }

      let baseUsername = (user.displayName || user.email.split("@")[0]).replace(/\s+/g, "").toLowerCase();
  ...
  return (
    <dialog open className="w-[100%] flex justify-center">
      <div className="relative border w-[45%] m-4 min-h-[500px] rounded-3xl p-2">
        <button
          onClick={onClose}
          className="absolute border rounded-full hover:bg-gray-200 p-[2px] size-[30px] top-2 right-2 cursor-pointer"
        >
          ✖️
        </button>
        ...
        <div className="w-[75%] mt-10 h-[500px] justify-self-center text-left text-2xl flex-row">
          <form>
            ...
            <select
              ...
              value={loginChoice}
              onChange={(e) => SetLoginChoice(e.target.value as "Email" | "UserName")}
            >
              <option>Email</option>
            ...
            <button
              className="w-full bg-gray-100 text-black py-2 rounded hover:bg-gray-200 mb-4"
            ...
            >
              If you don't have an account Register instead
            </Link>
          </form>
        </div>
      </div>
